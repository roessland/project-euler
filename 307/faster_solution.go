// Runs in 30 seconds on an i7-4770k. Probably needs 16 GB RAM.
package main
...
func P(ballsLeft, zeroBins, oneBins int32) float64 {
	...
	smallpZero := pZero < 0.00000001
	smallpOne := pOne < 0.00000001

	var prob float64
	if smallpZero && smallpOne {
		prob = 0.0
	} else if smallpZero {
		prob = P(ballsLeft-1, zeroBins, oneBins-1) * pOne
	} else if smallpOne {
		prob = P(ballsLeft-1, zeroBins-1, oneBins+1) * pZero
	} else {
